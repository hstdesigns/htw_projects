...
namespace htw_gimbal_debug
{
    ...
    public partial class Form_3Dcuboid : Form
    {
        ...
        //static MadgwickAHRS AHRS = new MadgwickAHRS(1f / 10.0f, 0.2f);
        static MahonyAHRS AHRS = new MahonyAHRS(1f / 25f, 1f, 0f);
        ...
        {
            float accX = (float)fmMain.p3d_a.x;
            float accY = (float)fmMain.p3d_a.y;
            float accZ = (float)fmMain.p3d_a.z;

            float gyroX = (float)fmMain.p3d_g.x;
            float gyroY = (float)fmMain.p3d_g.y;
            float gyroZ = (float)fmMain.p3d_g.z;

            float magX = (float)fmMain.p3d_m.x;
            float magY = (float)fmMain.p3d_m.y;
            float magZ = (float)fmMain.p3d_m.z * -1.0f;

            lock (this)
            {
                //AHRS.Update(0.52f, 0.52f, 0.52f, 0.0f, 0.1f, 0.0f);
                AHRS.Update(deg2rad(gyroX), deg2rad(gyroY), deg2rad(gyroZ), accX, accY, accZ);
                //AHRS.Update(deg2rad(gyroX), deg2rad(gyroY), deg2rad(gyroZ), accX, accY, accZ, magX, magY, magZ);
