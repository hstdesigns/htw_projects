...
namespace x_IMU_IMU_and_AHRS_Algorithms
{
    ...
    public partial class Form_3Dcuboid : Form
    {
        ...
        static SerialPort sp = new SerialPort("COM4", 9600, Parity.None, 8, StopBits.One);
        ...
        //static AHRS.MadgwickAHRS AHRS = new AHRS.MadgwickAHRS(1f / 20f, 0.2f);
        static AHRS.MahonyAHRS AHRS = new AHRS.MahonyAHRS(1f / 20f, 1f, 0f);
        ...
        {
            degrees *= 500.0f / 65536.0f;
            return (float)(Math.PI / 180) * degrees;
        }

        private void formUpdateTimer_Tick_OPT(object sender, EventArgs e)
        {
            sp.BaseStream.WriteByte(8);
            ...
            q[0] = float.Parse(sl[0]);
            q[1] = float.Parse(sl[1]);
            q[2] = float.Parse(sl[2]);
            q[3] = float.Parse(sl[3]);

            lock (this)
            {
                float w, x, y, z, pitch, roll, yaw; 



                //w = (float)Math.Sqrt(w * w + x * x + y * y + z * z);
                ...
                RotationMatrix = (new x_IMU_API.QuaternionData(q)).ConvertToConjugate().ConvertToRotationMatrix();
                ...
                //yaw = (float)Math.Atan2(2.0 * (y * z + w * x), w * w - x * x - y * y + z * z);
                //pitch = (float)Math.Asin(-2.0 * (x * z - w * y));
                //roll = (float)Math.Atan2(2.0 * (x * y + w * z), w * w + x * x - y * y - z * z);
